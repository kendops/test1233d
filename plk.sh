...
verify_uninstall() {
    ...
    for USER in awx pulp automation-controller; do
        ssh "$NODE" "if id '$USER' &>/dev/null; then echo '❌ User $USER still exists'; else echo '✅ User $USER is removed'; fi"
    done

    for GROUP in awx pulp automation-controller; do
        ssh "$NODE" "if getent group '$GROUP' &>/dev/null; then echo '❌ Group $GROUP still exists'; else echo '✅ Group $GROUP is removed'; fi"
    done
    ...
    ssh "$NODE" "if [ -d /var/lib/pulp ] && [ \"\$(ls -A /var/lib/pulp 2>/dev/null)\" ]; then echo '❌ /var/lib/pulp is NOT empty'; else echo '✅ /var/lib/pulp is completely empty'; fi"
