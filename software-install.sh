...
echo "Creating a JavaScript project directory..." | tee -a "$LOG_FILE"
...
echo "Installing Vite and Gatsby..." | tee -a "$LOG_FILE"
npm install -g vite@latest 2>&1 | tee -a "$LOG_FILE"
npm install -g gatsby-cli@latest 2>&1 | tee -a "$LOG_FILE"
...
echo "Checking Vite and Gatsby versions..." | tee -a "$LOG_FILE"
vite_version=$(vite --version 2>/dev/null)
gatsby_version=$(gatsby --version 2>/dev/null)

if [[ $? -eq 0 ]]; then
    echo "Vite Version: $vite_version" | tee -a "$LOG_FILE"
else
    echo "Vite is not installed or not configured correctly." | tee -a "$LOG_FILE"
fi

if [[ $? -eq 0 ]]; then
    echo "Gatsby Version: $gatsby_version" | tee -a "$LOG_FILE"
else
    echo "Gatsby is not installed or not configured correctly." | tee -a "$LOG_FILE"
fi
...
echo "------------------------------------------------" | tee -a "$LOG_FILE"
for package in react react-dom react-router-dom react-hot-toast @mui/material moment axios react-icons react-hook-form next jwt-decode vite gatsby; do
